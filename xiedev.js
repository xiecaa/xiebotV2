...
* Jangan Di Jual!
* Ini Scpirt Free Buat Kalian, Tapi Lihat Dibawah!
	...
	xiedev.on('chat-update', async (team) => {
         ...
         const readMore = more.repeat(4001)
	      ...
	      const fakeReply = (teks) => {
            xiedev.sendMessage(from, teks, text, {quoted: {key: {fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "6283818221226-1616169743@g.us" } : {}) }, message: {"imageMessage": {"url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc","mimetype": "image/jpeg","caption": ${namabot},"fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=","fileLength": "28777","height": 1080,"width": 1079,"mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=","fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=","directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69","mediaKeyTimestamp": "1610993486","jpegThumbnail": fs.readFileSync('./src/fotobot2.jpg'),"scansSidecar": "1W0XhfaAcDwc7xh1R8lca6Qg/1bB4naFCSngM2LKO2NoP5RI7K+zLw=="}}}})
         }
			const replyyy = (teks) => {
         ...
         return fakeReply('Mohon Jangan Spam\nKasih Waktu 6 Detik!')
         }
         ...
         if (budy.startsWith('=-')){
         try {
     	   if (!isOwner && !team.key.fromMe) return fakeReply(mess.only.ownerB)
         ...
         public = false
         fakeReply(`Success`, `Status : Self`)
         }
         if (chats.toLowerCase() == 'status'){
         fakeReply(`STATUS : ${public ? 'Public' : 'Self'}`)
         }
         }
         if (!public){
...
//>>>>>>>>>[ KHUSUS INFO BOT ]<<<<<<<<<<\\
         ...
         case 'author':
              ...
              await sleep(3000)
              xiedev.sendMessage(from,'Ini Kak Kontaknya🤗',text,{quoted: hhehe})
              break
         case 'status':
              if (!isUser) return replyy(from, dev1, dev2, dev3, {quoted: ftroli})
			     if (isBanned) return replyy(from, team1, team2, team3, {quoted: ftroli})
...
//>>>>>>>>>[ KHUSUS CONVERT ]<<<<<<<<<<\\
         ...
         case 'tts':
              ...
              if (err) return reply(mess.error.eror)
              xiedev.sendMessage(from, buffer, audio, {quoted: team, ptt:true, duration: 999990000})
              fs.unlinkSync(rano)
              })
              })
              break
...
//>>>>>>>>>[ KHUSUS OWNER ]<<<<<<<<<<\\
         case 'self':
              if (!isOwner && !team.key.fromMe) return fakeReply(mess.only.ownerB)
              public = false
              return fakeReply('*MODE : SELF*')
              break
			case 'public':
              if (!isOwner && !team.key.fromMe) return fakeReply(mess.only.ownerB)
              public = true
              return fakeReply('*MODE : PUBLIC*')
              break
         case 'banchat':
         case 'mute':
              if (!isOwner && !team.key.fromMe) return fakeReply(mess.only.ownerB)
              if (isBanchat) return fakeReply(`Sudah Mute Grup`)
              bancht.push(from)
         ...
         case 'unmute':
              if (isBanchat){
              if (!isOwner && !team.key.fromMe) return fakeReply(mess.only.ownerB)
              if (!isBanchat) return fakeReply(`Sudah Unmute Grup`)
              ...
              break
         case 'setprefix':
              if (!isOwner) return fakeReply(mess.only.ownerB)
              if (args.length < 1) return reply('Textnya Mana')
              prefix = args[0]
              fakeReply(`Menjadi : ${prefix}`)
              break
         case 'sethias':
         case 'sethiasan':
              if (!isOwner) return fakeReply(mess.only.ownerB)
         ...
         case 'yahaha':
              if (!isOwner && !team.key.fromMe) return fakeReply(mess.only.ownerB)
              await xiedev.toggleDisappearingMessages(from, 0)
              await xiedev.toggleDisappearingMessages(from, 0)
              fakeReply("Hai Bang")
              break
         case 'clearall':
         ...
         case 'deletechat':
              if (!isOwner && !team.key.fromMe) return fakeReply(mess.only.ownerB)
              fakeReply('Sukses Menghapus Chat' + from)
              setTimeout( () => {
         ...
         case 'ban': // Error
				  ...
				  mentioned = team.message.extendedTextMessage.contextInfo.mentionedJid
			     ban = mentioned
			     fakeReply(`Berhasil Banned : ${ban}`)
				  break
         case 'unban': // Error
				  if (!isOwner && !team.key.fromMe) return fakeReply(mess.only.ownerB)
				  bnnd = body.slice(8)
				  ban.splice(`${bnnd}@s.whatsapp.net`, 1)
			     fakeReply(`Nomor wa.me/${bnnd} telah di unban!`)
				  break
         case 'block':
				  xiedev.updatePresence(from, Presence.composing) 
				  if (!isOwner && !team.key.fromMe) return fakeReply(mess.only.ownerB)
				  xiedev.blocked (`${body.slice(7)}@c.us`, "add")
				  fakeReply(`Perintah Diterima, Memblokir ${body.slice(7)}@c.us`)
				  break
         case 'unblock':
				  if (!isOwner && !team.key.fromMe) return fakeReply(mess.only.ownerB)
				  xiedev.blocked (`${body.slice(9)}@c.us`, "remove")
				  fakeReply(`perintah Diterima, Membuka Blokir ${body.slice(9)}@c.us`)
				  break 
         case 'scmd':
         case 'stickscmd':
              if (!isOwner && !team.key.fromMe) return fakeReply(mess.only.ownerB)
         ...
         case 'autoread':
              ...
              fakeReply(`*List Auto Read*\n• gc\n• pc`)
              }
              break
         case 'autojoin':
              if (!isOwner && !team.key.fromMe) return fakeReply(mess.only.ownerB)
              if ((args[0]) === 'on') {
              if (autojoin == true) return fakeReply('Auto-Join Sudah On')
              autojoin = true
              fakeReply(mess.sukses)
              } else if ((args[0]) === 'off') {
              autojoin = false
              ...
              fakeReply('On Mengaktifkan\nOff Menonaktifkan')
              }
              break
         case 'welcomeow':
              if (!isOwner && !team.key.fromMe) return fakeReply(mess.only.ownerB)
              if (!isGroup) return replyyy(mess.only.group)
              if (args.length < 1) return fakeReply('On Mengaktifkan\nOff Menonaktifkan')
              if ((args[0]) === 'on') {
              if (isWelkom) return fakeReply('Welcome Sudah On')
              welkom.push(from)
              fs.writeFileSync('./database/welkom.json', JSON.stringify(welkom))
              fakeReply(mess.sukses)
              } else if ((args[0]) === 'off') {
              if (isWelkom) return fakeReply('Welcome Sudah Off')
              welkom.splice(from, 1)
              fs.writeFileSync('./database/welkom.json', JSON.stringify(welkom))
              fakeReply(mess.sukses)
              } else {
         ...
         case 'product':
              ...
              break
         case 'product2':
              if (!isOwner && !team.key.ftomMe) return fakeReply(mess.only.ownerB)
